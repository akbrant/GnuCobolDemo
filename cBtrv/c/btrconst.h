...
#define LONGER_24BYTE_OWNER_NAME_LEN      24                            /* This does not include a NULL terminator */
#define LONGER_OWNER_NAME_LEN             32                            /* This does not include a NULL terminator */
#define LONGER_HEX_OWNER_NAME_LEN         66                            
#define LONGER_OWNER_NAME_BUFFER_SIZE     LONGER_OWNER_NAME_LEN + 1     /* Includes a NULL terminator & type byte */
#define LONGER_HEX_OWNER_NAME_BUFFER_SIZE LONGER_HEX_OWNER_NAME_LEN + 1 /* Includes a NULL terminator & type byte */
#define HEX_OWNER_NAME_MIN_SIZE           32

#define POS_BLOCK_SIZE        128
#define PHYSICAL_POS_BUF_LEN  4          /* data buf size for Get Position */
#define BTRVEXID_PHYSICAL_POS_BUF_LEN  8 /* data buf size for Get Position using BTRVEXID */
...
#define GET_DRTC_XTRACTOR_KEY    -2
...
#define B_LOGIN             78
#define B_GET_NEXT_EXT_DEL  85
#define B_GET_PREV_EXT_DEL  86
#define B_STEP_NEXT_EXT_DEL 87
#define B_STEP_PREV_EXT_DEL 88
#define B_LOGOUT            B_LOGIN

/***************************************************************************
                           Operation Bias Codes
***************************************************************************/
#define S_WAIT_LOCK    100    /* bias for single record wait lock           */
#define S_NOWAIT_LOCK  200    /* bias for single record nowait lock         */
#define M_WAIT_LOCK    300    /* bias for multiple record wait lock         */
#define M_NOWAIT_LOCK  400    /* bias for multiple record nowait lock       */

#define WAIT_T         (B_BEGIN_TRAN + S_WAIT_LOCK)   /* 119 begin transaction with wait (same as 19)   */
#define NOWAIT_T       (B_BEGIN_TRAN + S_NOWAIT_LOCK) /* 219 begin transaction with nowait              */
#define WAIT3_T        (B_BEGIN_TRAN + M_WAIT_LOCK)   /* 319 begin transaction with wait (same as 19)   */
#define NOWAIT4_T      (B_BEGIN_TRAN + M_NOWAIT_LOCK) /* 419 begin transaction with nowait              */

#define NOWRITE_WAIT   500    /* function code bias when ins/del/upd should not retry */
#define CCURR_T_BIAS  1000    /* function code bias for concurrent trans    */
#define NESTED_T_BIAS 2000    /* bias for nested trans                      */
...
#define CREATEMODE_NORMAL                      0  /* Create Normal file - even if exists */
#define CREATEMODE_NEW                        -1  /* Create Normal file - Return Status 59 if file already exists */
#define CREATEMODE_RENAME_FILE              -127  /* Rename an existing file in the data buffer to the name in the key buffer */
#define CREATEMODE_DELETE_FILE              -128  /* Delete a file */

#define IGNORE_KEY                        -1       /* ignore the key number */
#define ALTERNATE_STAT_BUF                -1             /* use with B_STAT */

/***************************************************************************
                           Btrieve File Open Modes
***************************************************************************/
#define B_NORMAL            0   /* normal mode        */
#define ACCELERATED        -1   /* accelerated mode   */
#define MINUSONE           -1   /* byte value for -1  */
#define READONLY           -2   /* read only mode     */
#define WRITEABLE          -3   /* writeable file     */
#define EXCLUSIVE          -4   /* exclusive mode     */
...
#define AUTOTIMESTAMP_TYPE   32
#define TIMESTAMP2_TYPE      34
